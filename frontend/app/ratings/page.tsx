...
function RatingCard({
  ...
  return (
    <motion.div
      ...
      className="bg-white border border-gray-100 rounded-lg overflow-hidden group shadow-sm"
    >
      <div className="flex">
        {/* Poster */}
        ...
        <div className="flex-1 p-4 flex flex-col justify-between min-w-0">
          <div>
            ...
            <Link href={`/movies/${movie.id}`}>
              <h3 className="text-foreground font-medium text-base sm:text-lg truncate hover:text-primary-400 transition">
                {displayTitle}
              </h3>
            </Link>

            {/* Meta */}
            <div className="flex items-center space-x-2 text-xs sm:text-sm text-foreground/50 mt-1">
              {year && <span>{year}</span>}
            ...
            {/* Date and Weather */}
            <div className="flex flex-wrap items-center gap-3 mt-2 text-xs text-foreground/50">
              <div className="flex items-center space-x-1">
          ...
          <div className="flex justify-end mt-2">
            <button
              ...
              disabled={isDeleting}
              className="p-2 text-foreground/40 hover:text-primary-500 hover:bg-primary-500/10 rounded-lg transition opacity-0 group-hover:opacity-100"
              title="평점 삭제"
            >
              {isDeleting ? (
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          <div className="flex items-center space-x-3 mb-2">
            ...
            <h1 className="text-3xl font-bold text-foreground">내 평점</h1>
          </div>
          <p className="text-foreground/60">
            {loading ? (
              ...
              <span>
                총 <span className="text-foreground font-medium">{stats.totalCount}개</span> 영화 ·
                평균 <span className="text-yellow-500 font-medium">{stats.averageScore.toFixed(1)}</span>점
              </span>
            ) : (
              "아직 평점을 남긴 영화가 없습니다"
            )}
        ...
        <AnimatePresence mode="wait">
          {loading ? (
            ...
            >
              {[...Array(5)].map((_, i) => (
                <div key={i} className="bg-white border border-gray-100 rounded-lg p-4 animate-pulse shadow-sm">
                  <div className="flex space-x-4">
                    <div className="w-24 sm:w-32 aspect-[2/3] bg-gray-200 rounded" />
            ...
            >
              ...
              {recommendedMovies.length > 0 && (
                <div className="mt-8">
                  <h4 className="text-lg font-semibold text-foreground mb-4 text-left">
                    이런 영화는 어떠세요?
                  </h4>
              ...
              {hasMore && (
                <div className="flex justify-center mt-8">
                  <button
                    ...
                    className="px-8 py-3 bg-foreground/10 hover:bg-foreground/20 disabled:opacity-50 text-foreground rounded-lg transition flex items-center space-x-2"
                  >
                    {loadingMore ? (
                      <>
